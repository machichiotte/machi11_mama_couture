  ...
  <div class="flex flex-col gap-6">
    ...
    <div class="relative group aspect-[4/5] md:max-h-[70vh] bg-secondary/30 overflow-hidden rounded-sm border border-primary/5 cursor-zoom-in" @click="openLightbox(activeIndex)">
      ...
      <div v-if="images.length > 1" class="absolute inset-x-4 top-1/2 -translate-y-1/2 flex justify-between opacity-0 group-hover:opacity-100 transition-opacity">
        <button @click.stop="prevImage" class="w-10 h-10 rounded-full bg-primary/90 backdrop-blur-sm border border-secondary/20 flex items-center justify-center hover:bg-primary text-secondary transition-all shadow-lg hover:shadow-xl">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7" /></svg>
        </button>
        <button @click.stop="nextImage" class="w-10 h-10 rounded-full bg-primary/90 backdrop-blur-sm border border-secondary/20 flex items-center justify-center hover:bg-primary text-secondary transition-all shadow-lg hover:shadow-xl">
          <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7" /></svg>
        </button>
      </div>

      <!-- Badge Zoom -->
      <div class="absolute bottom-4 right-4 p-2 bg-primary/60 backdrop-blur-sm rounded-full opacity-0 group-hover:opacity-100 transition-opacity">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4 text-secondary" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0zM10 7v3m0 0v3m0-3h3m-3 0H7" /></svg>
      </div>
    </div>
